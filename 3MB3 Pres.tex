...
\author[Jaswal, Khan, Palin, Reeves, and Xu]{Zeyn Jaswal, Azzaam Khan, Timothy Palin, Meredith Reeves, Tony Xu}
...
\begin{frame}{Overview}
   ...
   \vspace{0.3cm} 
    ...
    \begin{itemize}
        ...
        \item How do limited resources affect defence spending?
    \end{itemize}

\vspace{0.3cm}

    \textbf{Research Question:} What happens to their arms spending over time? 
...
\end{equation}
    ...
    \begin{itemize}
        ...
        \item \(b,d\): Spending Burden (\(b,d>0\))
...
\begin{frame}{Long Term Behavior ($r=s=0$)}
...
\begin{frame}{Long Term Behavior with Trust/Hostility}
...
\begin{frame}{Long-Term Behavior of Mutual Logistic Fear}

\begin{columns}[T]
    \column{0.33\textwidth}
    \centering
    \textbf{$bd < ac,\; r < 0,\; s < 0$}\\
    \includegraphics[width=\linewidth]{logistic_rs_case1.png}\\
    \small Mutual disarmament

    \column{0.33\textwidth}
    \centering
    \textbf{$bd = ac,\; r < 0,\; s > 0$}\\
    \includegraphics[width=\linewidth]{logistic_rs_case2.png}\\
    \small Stabilized spending

    \column{0.33\textwidth}
    \centering
    \textbf{$bd > ac,\; r > 0,\; s > 0$}\\
    \includegraphics[width=\linewidth]{logistic_rs_case3.png}\\
    \small Stabilized spending
\end{columns}
...
% \begin{itemize}
%     \item Adding $r$ and $s$ shifts the system through exogenous trust/hostility effects.
%     \item In the logistic model, finite carrying capacities $K_p$ and $K_g$ prevent unbounded growth.
%     \item As a result, long-run behavior is shaped not only by $bd$ versus $ac$, but also by the signs and magnitudes of $r$ and $s$.
% \end{itemize}
