...
\begin{frame}{Long Term Behavior (Base Model)}
    First Look at countries that are neutral, r=s=0
    \begin{columns}
...
\vspace{0.1cm}
        ...
        \begin{itemize}
            \item \textbf{Base Model:} behaviour based on ratio $bd$ vs $ac$
            \item \textbf{Public Sentiment:} model behaviour can now change based on initial spending, also impacts equilibrium so stable positive spending is more common
            \item \textbf{Mutual Logistic Fear:} model now accounts for economic limitations, preventing unending arms race to infinite spending
        \end{itemize}
        \end{block}
        
        \begin{block}{Next Steps (In Report)}
        \begin{itemize}
